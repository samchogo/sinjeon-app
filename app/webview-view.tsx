...
export default function WebviewViewScreen() {
  ...
  const injectedFlutterShimJs = `(() => { try { if (!window.flutter_inappwebview) window.flutter_inappwebview = {}; var bridge = window.flutter_inappwebview; bridge.callHandler = function(name){ try { var n = String(name || ''); if (n === 'loginCompleteHandler') { try { if (typeof window.requestFcmToken === 'function') { return window.requestFcmToken().then(function(r){ try { var token = r && r.token ? String(r.token) : ''; var osTypeCd = r && r.osTypeCd ? String(r.osTypeCd) : ((/iPad|iPhone|iPod/i.test(navigator.userAgent)) ? 'IOS' : 'ANDROID'); var deviceUid = token; return { code: '0000', result: { token: token, deviceUid: deviceUid, osTypeCd: osTypeCd } }; } catch(e){ return { code: '9999', result: null }; } }).catch(function(){ return { code: '9999', result: null }; }); } return new Promise(function(resolve){ try { var id = String(Date.now()) + '_' + Math.random().toString(36).slice(2); var done = false; var t = setTimeout(function(){ if (!done) resolve({ code: '9999', result: null }); }, 15000); var orig = window.__onNativeFcmToken; window.__onNativeFcmToken = function(payload){ try { if (payload && payload.id === id) { done = true; try{ clearTimeout(t); }catch(_){ } var token = payload && payload.token ? String(payload.token) : ''; var osTypeCd = (payload && payload.osTypeCd) || ((/iPad|iPhone|iPod/i.test(navigator.userAgent)) ? 'IOS' : 'ANDROID'); resolve({ code: '0000', result: { token: token, deviceUid: token, osTypeCd: osTypeCd } }); window.__onNativeFcmToken = orig || window.__onNativeFcmToken; return; } } catch(_){ } try { if (typeof orig === 'function') orig(payload); } catch(_){ } }; window.ReactNativeWebView && window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'REQUEST_FCM_TOKEN', id: id })); } catch(_){ resolve({ code: '9999', result: null }); } }); } return Promise.resolve(null); } catch(e){ return Promise.resolve(null); } }; } catch(e){} })(); true;`;
  const injectedFlutterShimJs = `(() => { try { if (!window.flutter_inappwebview) window.flutter_inappwebview = {}; var bridge = window.flutter_inappwebview; bridge.callHandler = function(name){ try { var n = String(name || ''); if (n === 'loginCompleteHandler') { try { if (typeof window.requestFcmToken === 'function') { return window.requestFcmToken().then(function(r){ try { var token = r && r.token ? String(r.token) : ''; var osTypeCd = r && r.osTypeCd ? String(r.osTypeCd) : ((/iPad|iPhone|iPod/i.test(navigator.userAgent)) ? 'IOS' : 'ANDROID'); var deviceUid = token; return { code: '0000', result: { token: token, deviceUid: deviceUid, osTypeCd: osTypeCd } }; } catch(e){ return { code: '9999', result: null }; } }).catch(function(){ return { code: '9999', result: null }; }); } return new Promise(function(resolve){ try { var id = String(Date.now()) + '_' + Math.random().toString(36).slice(2); var done = false; var t = setTimeout(function(){ if (!done) resolve({ code: '9999', result: null }); }, 15000); var orig = window.__onNativeFcmToken; window.__onNativeFcmToken = function(payload){ try { if (payload && payload.id === id) { done = true; try{ clearTimeout(t); }catch(_){ } var token = payload && payload.token ? String(payload.token) : ''; var osTypeCd = (payload && payload.osTypeCd) || ((/iPad|iPhone|iPod/i.test(navigator.userAgent)) ? 'IOS' : 'ANDROID'); resolve({ code: '0000', result: { token: token, deviceUid: token, osTypeCd: osTypeCd } }); window.__onNativeFcmToken = orig || window.__onNativeFcmToken; return; } } catch(_){ } try { if (typeof orig === 'function') orig(payload); } catch(_){ } }; window.ReactNativeWebView && window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'REQUEST_FCM_TOKEN', id: id })); } catch(_){ resolve({ code: '9999', result: null }); } }); } if (n === 'getDeviceLocationHandler') { try { return new Promise(function(resolve){ try { var done=false; var t=setTimeout(function(){ if(!done) resolve({ code:'9999', result:null }); },15000); var onOk=function(pos){ try{ done=true; try{ clearTimeout(t);}catch(_){ } var c=pos&&pos.coords?pos.coords:{}; resolve({ code:'0000', result:{ latitude: c.latitude || null, longitude: c.longitude || null } }); }catch(_){ resolve({ code:'9999', result:null }); } }; var onErr=function(){ try{ done=true; clearTimeout(t);}catch(_){ } resolve({ code:'9999', result:null }); }; if (navigator && navigator.geolocation && typeof navigator.geolocation.getCurrentPosition==='function'){ navigator.geolocation.getCurrentPosition(onOk, onErr); } else { resolve({ code:'9999', result:null }); } } catch(_){ resolve({ code:'9999', result:null }); } }); } catch(e){ return Promise.resolve({ code:'9999', result:null }); } } return Promise.resolve(null); } catch(e){ return Promise.resolve(null); } }; } catch(e){} })(); true;`;
  const injectedCommonFlutterJs = `(() => { try { if (!window.CommonFlutterInterface) window.CommonFlutterInterface = {}; var bridge = window.CommonFlutterInterface; bridge.postMessage = function(message){ try { var msgStr = (message == null) ? '' : String(message); var obj = {}; try { obj = JSON.parse(msgStr); } catch(_) {} var t = obj && obj.type ? String(obj.type) : ''; if (t === 'OPEN_EXTERNAL_BROWSER') { var url = (obj && obj.data && obj.data.url) ? String(obj.data.url) : (obj && obj.url) ? String(obj.url) : ''; if (url) { try { window.ReactNativeWebView && window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'OPEN_EXTERNAL_LINK', url: url })); } catch(_){} } return; } } catch(_){ } }; } catch(e){} })(); true;`;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <WebView
        ...
        onShouldStartLoadWithRequest={(req) => {
          ...
          if (u.startsWith('sinjeonapp://close_webview')) { backOrHome(); return false; }
...
const styles = StyleSheet.create({
  ...
  headerButton: { width: 44, alignItems: 'center', justifyContent: 'center',color: '#000' },
  headerButtonText: { fontSize: 22 },
  headerTitle: { flex: 1, textAlign: 'center', fontSize: 16, fontWeight: '600',color: '#000' },
